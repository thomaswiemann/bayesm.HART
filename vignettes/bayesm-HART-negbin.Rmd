---
title: "Hierarchical Negative Binomial Model"
date: "2025-06-29"
output: rmarkdown::html_vignette
vignette: |
  %\VignetteIndexEntry{Hierarchical Negative Binomial Model}
...
This vignette estimates a hierarchical negative binomial model with HART priors using the `bayesm.HART` package, applied to the `detailing` pharmaceutical panel dataset from `bayesm`. The HART prior specifies the *representative physician* as a flexible, non-parametric function of observed physician characteristics via a sum-of-trees model $\Delta(Z_i)$, in contrast to the conventional linear specification $\Delta^\top Z_i$.

## Pharmaceutical Detailing Data

The `detailing` dataset contains a balanced panel of 1,000 physicians observed over 23 months. For each physician-month, we observe prescriptions written (`scripts`), detailing visits (`detailing`), and lagged prescriptions (`lagged_scripts`). Physician demographics (`generalphys`, `specialist`, `mean_samples`) serve as covariates $Z_i$.
...
We fit both the HART model (Wiemann, 2025) and a conventional linear hierarchical model (Rossi et al., 2009). Both estimate physician-specific coefficients $\beta_i$ via hierarchical negative binomial regression with shared overdispersion $\alpha$; they differ only in their specification of $\Delta(\cdot)$. Note that `rhierNegbinRw` uses Metropolis-Hastings (the negative binomial likelihood is not conjugate), so a longer chain is recommended.
...
<img src="figures/negbin-mcmc-diagnostics-1.png" alt="MCMC Traceplot of the Log Likelihood." width="90%" />
...
<div class="figure" style="text-align: center">
<img src="figures/negbin-individual-betas-1.png" alt="Posterior Distributions of Physician-level Detailing Coefficients." width="90%" />
<p class="caption">Posterior Distributions of Physician-level Detailing Coefficients.</p>
</div>
...
#> 1 Linear       840 0.00804 0.1219
#> 2 Linear       888 0.01589 0.0769
#> 3   HART       840 0.04243 0.1242
#> 4   HART       888 0.02002 0.0767
```

Both models produce similar individual-level estimates for these physicians. With 23 monthly observations per physician, the individual-level posterior is dominated by the physician's own data rather than the hierarchical prior.

## Posterior Inference on Physician Segment Coefficients

The models differ in how they characterize the expected coefficient $\Delta(Z)$ for a representative physician with demographics $Z$. We compare these segment-level predictions for the same two physicians, using the `predict` function.
...
<div class="figure" style="text-align: center">
<img src="figures/negbin-representative-physician-1.png" alt="Posterior Distributions of Expected Physician Segment Coefficients." width="90%" />
<p class="caption">Posterior Distributions of Expected Physician Segment Coefficients.</p>
</div>
...
#> 1               Segment 1 (GP, No Samples)      0.0190    0.0115    0.0342
#> 2 Segment 2 (Specialist, Moderate Samples)      0.0163    0.0195    0.0255
#>   HART_SD
#> 1  0.0171
#> 2  0.0284
...
#> Linear approach difference: -0.002788167
cat("HART approach difference:", 
    deltaZ_summary$HART_Mean[2] - deltaZ_summary$HART_Mean[1], "\n")
#> HART approach difference: -0.008672027
```

The HART segment-level predictions align with the individual-level posteriors above. The linear approach is constrained to a linear function of demographics, while the HART model can capture nonlinearities and interactions.
