...
```{r, echo = FALSE}
vignette_cache_dir <- file.path(tools::R_user_dir("bayesm.HART", which = "cache"), "vignettes")
dir.create(vignette_cache_dir, recursive = TRUE, showWarnings = FALSE)
...
projection on a small set of characteristics. The same framework supports
prediction for new consumers and posterior updating for consumers with
accumulating choice histories.
...
Ginter (1995) bank conjoint dataset. The first code block loads the data,
formats the list structure required by `rhierMnlRwMixture`, and sets MCMC
hyperparameters. This setup syntax is the same in `bayesm` and
`bayesm.HART`.
...
```{r, include = FALSE}
# Cache wrapper — fit only if needed
model_cache_file <- file.path(
  vignette_cache_dir,
  "readme-bank-R2000-keep2.rds"
)

model_cache_ok <- FALSE
if (file.exists(model_cache_file)) {
  model_cache <- tryCatch(readRDS(model_cache_file), error = function(e) NULL)
  model_cache_ok <- !is.null(model_cache) && !is.null(model_cache$out)
  if (model_cache_ok) out <- model_cache$out
}
```
...
estimand. A central object is the *representative respondent*, i.e., expected
part-worths conditional on respondent characteristics. The following code
computes posterior means and standard deviations for three credit card
attributes.
