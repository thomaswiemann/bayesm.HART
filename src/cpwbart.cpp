...
#ifdef _OPENMP
#include <omp.h> // Need for OpenMP functions
...
)
{
   ...
   ttss >> nd >> m >> p;
   if (verbose) {
        Rcpp::Rcout << "number of bart draws: " << nd << std::endl;
        Rcpp::Rcout << "number of trees in bart sum: " << m << std::endl;
   ...
   size_t pp = ixi.size();
   if(p!=pp && verbose) Rcpp::Rcout << "WARNING: p from trees and p from x don't agree\n";
   xinfo xi;
   xi.resize(p);
   ...
   size_t np = xpred.ncol(); // Note: R pwbart transposes, so ncol is npred
   if (verbose) Rcpp::Rcout << "from x, npred, ncov: " << xpred.nrow() << ", " << xpred.ncol() << std::endl;
   //--------------------------------------------------
   //read in trees
...
void local_getpred(size_t nd, size_t p, size_t m, size_t np, xinfo& xi, std::vector<vtree>& tmat, double *px, Rcpp::NumericMatrix& yhat)
{
   ...
   // Adjust end for the last thread to cover remaining draws
