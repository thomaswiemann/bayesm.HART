# Bayesian Hierarchical Linear Model with HART Prior

`rhierLinearMixture` implements an MCMC algorithm for a Bayesian
hierarchical linear regression model with a Hierarchical Additive
...
function of potentially many observed characteristics. `Prior$bart`
...
- Prior:
  ...
  - `nu.e` (optional): Degrees of freedom for error variance prior
    (default: 3).

  - `ssq` (optional): Scale for error variance prior, `nreg x 1` vector
    (default: var(y_i)).
  ...
  - `Ad` (optional): Prior precision for vec(Delta) (default: 0.01 \*
    I). Only used when `drawdelta = TRUE` and `useBART = FALSE`.

  - `deltabar` (optional): Prior mean for vec(Delta) (default: 0). Only
    used when `drawdelta = TRUE` and `useBART = FALSE`.
...
classes `"rhierLinearMixtureHeterCov"` and `"bayesm.HART.HeterCov"`.
...
[`predict.rhierLinearMixture()`](https://thomaswiemann.com/bayesm.HART/reference/predict.rhierLinearMixture.md),
[`marginal_effects.rhierLinearMixture()`](https://thomaswiemann.com/bayesm.HART/reference/marginal_effects.md)

## Author

Peter Rossi, Wayne Taylor (original bayesm code), Thomas Wiemann (HART
modifications).
