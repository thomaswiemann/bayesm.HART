# ==============================================================================
# Linear Predictive Engines
# ==============================================================================
# Internal helpers for linear posterior/prior predictive response simulation.

.predict_linear_posterior_response <- function(object, newdata,
                                               kept_draws_indices, r_verbose) {
  if (is.null(newdata$regdata) || !is.list(newdata$regdata)) {
    stop("newdata$regdata must be provided as a list for linear posterior predictions.")
  }
  ...
  for (i in seq_len(nreg)) {
    ...
    if (r_verbose) {
      cat("Simulating posterior responses for Unit", i, "of", nreg, "...\n")
    }
    ...
    for (s_orig in kept_draws_indices) {
      ...
      tau_is <- object$taudraw[s_orig, i]
      mu_is <- as.vector(X_i %*% beta_is)
      y_draws_i[, draw_count] <- rnorm(nobs_i, mean = mu_is, sd = sqrt(tau_is))
    }
    out[[i]] <- y_draws_i
  }

  if (r_verbose) cat("Linear posterior response simulation complete.\n")
  out
}

.predict_linear_prior_response <- function(object, newdata, delta_z_array,
                                           kept_draws_indices, nsim, r_verbose,
                                           hetercov_comps = NULL) {
  if (is.null(newdata$X) || !is.list(newdata$X)) {
    stop("newdata$X must be provided as a list for linear prior predictions.")
  }

  is_heter <- inherits(object, "bayesm.HART.HeterCov")
  if (!is_heter && !requireNamespace("bayesm", quietly = TRUE)) {
    stop("The 'bayesm' package is required for linear prior predictions. Please install it.")
  }
  ...
  if (is_heter) {
    ...
    .assert_single_component_nmix(object, "linear prior prediction")
  }

  tau_ref <- rowMeans(object$taudraw[kept_draws_indices, , drop = FALSE])
  ...
  for (i in seq_len(npred)) {
    ...
    if (r_verbose) {
      cat("Simulating prior responses for Prediction Unit", i, "of", npred, "...\n")
    }
    ...
    for (draw_idx in seq_len(ndraws_out)) {
      ...
      if (is_heter) {
        ...
        if (!is.null(sigma_kept)) {
          ...
          if (is.null(U)) {
            stop(sprintf("linear prior heter-cov failed at draw %d, unit %d: SigmaZ chol failed.",
                         s_orig, i))
          }
      ...
      if (nsim > 1L) {
        # Keep API shape stable while reducing MC noise for linear response path.
        eta_acc <- eta
        for (k in 2:nsim) {
          if (is_heter) {
            mu_s <- mu_kept[draw_idx, ]
      ...
      mu_y <- as.vector(X_i %*% beta_draw)
      y_draws_i[, draw_idx] <- rnorm(nobs_i, mean = mu_y, sd = sqrt(tau_ref[draw_idx]))
    }

    out[[i]] <- y_draws_i
  }

  if (r_verbose) cat("Linear prior response simulation complete.\n")
  out
}
